...
@section('content')
    <div class="bg-green-600 text-white py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center mb-4">
                <a href="{{ route('articles.index') }}" class="text-white hover:text-yellow-300 mr-4">
                ...
                <div>
                    <span
                        class="bg-yellow-500 text-white px-3 py-1 rounded text-sm font-medium uppercase">
                        {{ $article->kategori }}
                        </span>
                    <h1 class="text-4xl md:text-5xl font-bold mt-2">{{ $article->judul }}</h1>
                    </div>
                </div>
            <p class="text-xl opacity-90 text-yellow-100">{{ $article->created_at->format('d F Y') }}</p>
            </div>
        </div>
    ...
    <div class="py-16">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <article
                class="bg-white rounded-lg shadow-lg overflow-hidden border-b-4 border-transparent hover:border-yellow-400">
                @if ($article->gambar)
                        ...
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                <div class="flex space-x-3 mt-2">
                                    ...
                                    <a
                                        href="https://wa.me/?text={{ urlencode($article->judul . ' - ' . request()->fullUrl()) }}"
                                        target="_blank"
